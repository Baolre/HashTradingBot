...
class BalanceCurve(QWidget):
    ...
    def paintEvent(self, event):
        ...
        pts = [(10 + i * step, 10 + h - (v - mn) / (mx - mn) * h) for i, v in enumerate(data)]
        color = QColor(COLOR_EVEN) if data[-1] >= data[0] else QColor(COLOR_ODD)
...
class SimPanel(QWidget):
    ...
    def append_record(self, r: BetRecord) -> None:
        ...
        labels = {"odd":"单","even":"双","big":"大","small":"小"}
        self.table.setItem(0, 2, QTableWidgetItem(labels.get(r.bet_target, "?")))
        ...
        while self.table.rowCount() > 50: self.table.removeRow(self.table.rowCount() - 1)
