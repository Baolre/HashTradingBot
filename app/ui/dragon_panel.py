...
from ..core.analyzer import PARITY_EVEN, PARITY_ODD, SIZE_BIG, SIZE_SMALL, Analyzer, Period
from .theme import COLOR_BG, COLOR_BORDER, COLOR_EVEN, COLOR_ODD, COLOR_BIG, COLOR_SMALL, COLOR_PANEL, COLOR_SUB
...
class DragonEntry:
    ...
    @property
    def label(self) -> str:
        return {"odd": "单", "even": "双", "big": "大", "small": "小"}.get(self.value, "?")

    @property
    def color(self) -> str:
        return {"odd": COLOR_ODD, "even": COLOR_EVEN, "big": COLOR_BIG, "small": COLOR_SMALL}.get(self.value, COLOR_SUB)
...
class DragonPanel(QWidget):
    ...
    def scan_dragons(analyzer: Analyzer, rule_label: str = "", threshold: int = 3) -> List[DragonEntry]:
        ...
        for attr, dim in [("parity", "parity"), ("size", "size")]:
            val, run = analyzer.same_run_length(attr)
            valid_vals = (PARITY_ODD, PARITY_EVEN) if attr == "parity" else (SIZE_BIG, SIZE_SMALL)
            if run >= threshold and val in valid_vals:
                entries.append(DragonEntry(rule_label=rule_label, dimension=dim, value=val,
