...
class DataTablePanel(QWidget):
    def __init__(self, parent=None):
        ...
        self.table.setHorizontalHeaderLabels(["区块号", "哈希", "末位数字", "单双", "时间"])
    ...
    def refresh(self, analyzer: Analyzer) -> None:
        ...
        for i, p in enumerate(periods):
            ...
            pi = QTableWidgetItem(p.parity_label)
            pi.setForeground(QColor(COLOR_ODD if p.is_odd else COLOR_EVEN))
    ...
    def _open_tronscan(self, row, col):
        ...
        if item:
            QDesktopServices.openUrl(QUrl(f"https://tronscan.org/#/block/{item.text()}"))
    ...
    def _export(self):
        ...
        with open(path, "w", newline="", encoding="utf-8-sig") as f:
            w = csv.writer(f)
            w.writerow(["区块号", "哈希", "末位", "单双", "时间"])
            for p in periods:
                ts = datetime.fromtimestamp(p.timestamp_ms / 1000).strftime("%Y-%m-%d %H:%M:%S") if p.timestamp_ms else ""
                w.writerow([p.block_number, p.block_hash, p.digit, p.parity_label, ts])
