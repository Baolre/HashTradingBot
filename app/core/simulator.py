...
class Simulator:
    ...
    def on_new_period(self, period: Period, analyzer: Analyzer) -> Optional[BetRecord]:
        ...
        if not target:
            return None
        bet_amount = self._calc_bet()
        bet_amount = min(bet_amount, self.state.balance, self.cfg.max_bet)
        if bet_amount <= 0:
            self._running = False
            return None

        won = False
        if target in (PARITY_ODD, PARITY_EVEN):
            won = (target == period.parity)
            actual = period.parity
        else:
            won = (target == period.size)
            actual = period.size
        ...
        record = BetRecord(period_num=period.block_number, bet_amount=bet_amount,
                           bet_target=target, actual=actual, won=won, pnl=pnl,
                           balance_after=self.state.balance, strategy=self.cfg.strategy)
    ...
    def _select_target(self, analyzer: Analyzer) -> Optional[str]:
        ...
        if t == "follow_trend":
            ...
            if result.best and result.best.dimension == "parity":
    ...
    def _calc_bet(self) -> float:
        ...
        if s == "flat":
            ...
            return base * (2 ** max(0, abs(min(0, self.state.current_streak))))
        elif s == "dalembert":
            return base * (1 + max(0, self.state._dalembert_level))
        elif s == "fibonacci":
            return base * _FIB[min(self.state._fib_idx, len(_FIB) - 1)]
        elif s == "paroli":
            return base * (2 ** min(self.state._paroli_wins, 3)) if self.state._paroli_wins > 0 else base
        elif s == "kelly":
    ...
    def _update_strategy(self, won: bool) -> None:
        s = self.cfg.strategy
        if s == "dalembert":
